...
//3、ADC12_IN5电位器器初始化(PA5引脚)
void ADC1_IN5_PotentiometerInit(void)
{
	/* 0、外设信息配置结构体*/
	ADC_InitTypeDef       ADC_InitStructure;										//ADC外设基础信息配置结构体
	ADC_CommonInitTypeDef ADC_CommonInitStructure;									//ADC通用信息配置结构体
	...
	/* 2、配置ADC1通道5作为模拟输入 */
	GPIO_InitStructure.GPIO_Pin 	= GPIO_Pin_5;									//引脚：			第5个引脚
	GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_AN;									//模式：			模拟模式
	GPIO_InitStructure.GPIO_PuPd 	= GPIO_PuPd_NOPULL;								//是否上下拉：   不拉
	GPIO_Init(GPIOA, &GPIO_InitStructure);											//将配置的外设信息，写入到相对应的寄存器中
...
//4、获取电位器ADC值
uint16_t ADC1_IN5_GetPotentiometerVal(void)
{
	uint16_t ret_val = 0;
