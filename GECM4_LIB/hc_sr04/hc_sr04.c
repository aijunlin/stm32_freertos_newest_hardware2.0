...
/**
  ...
  * @note   GEC-M4板子上                      hc_sr04超声波模块
  *         CAMERA(13引脚)-》DCMI_D5-》PB6	  TRIG，触发信号引脚	     
  *         CAMERA(15引脚)-》DCMI_D7-》PE6    ECHO，回响信号引脚	   
  * @retval None
  */
void HC_SR04_Init(void)
{
	...
	// 3、初始化模块引脚的状态
	TRIG = 0;
	// ECHO = 0;											// 该引脚已经被设置成了输入模式，你设置不了电平的
}


/**
  * @brief  获取hc_sr04的超声波模块的距离
  * @note   获取的距离单位是毫米级
  * @param  None
  * @retval 成功：返回超声波获取的距离(浮点型数据)
  *                如果i使用的是声音在空气中的速度，返回为毫米级*****
  *                如果使用的是公式，返回位厘米级
  *         失败：
  *				返回非0
  */	
float HC_SR04_GetDis(void)
{
	...
	// 2、延时10us以上，让其高电平持续时间在10us以上
	delay_us(15);
	
	// 3、设置TRIG引脚为低电平
	TRIG = 0;
	
	// 二、检查ECHO引脚的状态，从而得知超声波的现在的清空			// 超声波模块：STM32老板，我现在在干活啦，你看一下我状态就知道了
	// 1、等待ECHO引脚从低电平变为高电平(模块开始发出超声波信号)
	count = 0;
	while( ECHO == 0)
	{
	...
	// 2、等待ECHO引脚从高电平变为低电平(模块接收到返回的超声波了)，并计数ECHO引脚在高电平的持续时间(超声波来回障碍物的时间)
	count = 0;
	while( ECHO == 1 )
	{
		count++;		// 计数有多少个8.8us(我们这里写8us)，就是计数有多少个3mm, 并测试其是否超时了
		delay_us(8);	
		if(count > 65000/8)
		{
			return -2;
	...
	dis_val = count*3.4/2.0;	// 利用在空气中声音的速度计算出(单位：毫米)
	// dis_val = coun/58.0;		// 利用公式(单位：厘米)
