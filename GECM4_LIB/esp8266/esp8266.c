...
int8_t ESP8266_OnLineInit(void)
{
	...
	printf("01: ESP8266_ExitTransparentTransmission success!\r\n");
	delay_ms(500);
	...
	printf("02: ESP8266_EnEcho success!\r\n");
	delay_ms(500);
	...
	printf("03: ESP8266_ConnectAp success!\r\n");
	delay_ms(500);
...
{
	// 1、清空相关数据
	MY_LIB_ClearArray((uint8_t*)u3_recvbuf, 128);
	u3_count = 0;
	u3_flag  = 0;
	u3_len   = 0;
...
{
	char *dest = str;
	char *src  = (char*)u3_recvbuf; // OK 0,CONNECT
